WD_PROP=${SPARQL}/kg-index/wikidata-properties-index.tsv
WD_ENT=${SPARQL}/kg-index/wikidata-entities-index.tsv
WD_EX=""
...
data:
	...
	--output ${SPARQL}/datasets/wikidata-simplequestions \
	--entity-index $(WD_ENT) \
	...
	--output ${SPARQL}/datasets/wikidata-lcquad2 \
	--entity-index $(WD_ENT) \
	--property-index $(WD_PROP) \
	--example-index $(WD_EX)
	...
	--output ${SPARQL}/datasets/wikidata-qald10 \
	--entity-index $(WD_ENT) \
	--property-index $(WD_PROP) \
	--example-index $(WD_EX)
	@echo "Preparing mcwq"
	@python scripts/prepare_data.py \
	--mcwq ${SPARQL}/mcwq \
	--output ${SPARQL}/datasets/wikidata-mcwq \
	--entity-index $(WD_ENT) \
	--property-index $(WD_PROP) \
	--example-index $(WD_EX)
	@echo "Preparing qa wiki"
	@python scripts/prepare_data.py \
	--qa-wiki ${SPARQL}/qa_wiki/qa_wiki.tsv \
	--output ${SPARQL}/datasets/wikidata-qa-wiki \
	--entity-index $(WD_ENT) \
	--property-index $(WD_PROP) \
	--example-index $(WD_EX)
...
example-indices:
	...
	--inputs ${SPARQL}/datasets/wikidata-*/train_input.txt \
	--targets ${SPARQL}/datasets/wikidata-*/train_sparql.txt \
	--output ${SPARQL}/example-index/wikidata.txt
	@echo "Building wikidata example index"
	@python scripts/build_vector_index.py \
	--data ${SPARQL}/example-index/wikidata.txt \
	--output ${SPARQL}/example-index/wikidata-$(MODEL) \
